...
const Boton = ({ name, onClick, style, type = "primary" }) => {
	...
	if (type === "submit") {
		return <Button type={type} htmlType="submit" onClick={onClick ? onClick : () => { console.log(`Click en ${name}`) }} style={{ ...estilo, ...style }}> {name}</Button >;
	}
	return <Button onClick={onClick} style={{ ...estilo, ...style }}>{name}</Button>;
};
